...
const reviewIssues: Array<{ source_id?: string; kind: string; detail: string; url?: string }> = [];
const observations: Array<{ source_id: string; status: string; http_status?: number }> = [];
...
function dateExpired(dateText: string): boolean {
  const end = new Date(`${dateText}T23:59:59.999Z`);
  return Number.isFinite(end.getTime()) && today.getTime() > end.getTime();
}
...
for (const source of registry.sources) {
  if (dateExpired(source.stale_after)) {
